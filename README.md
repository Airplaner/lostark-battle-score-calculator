...
## 각인
각 각인마다 전투력을 독립적으로 증가시키며, 읽은 각인서 등급과 어빌리티 스톤 장착 효과가 모두 반영된다.
...
공격력 증가 옵션은 기본 공격력이 높을 수록 효율이 떨어진다. 공격력 Lv.5 (+767)은 플레이어의 기본 공격력이 14만2037이라고 가정한 딜 증가량만큼 전투력이 증가한다.
...
공격력 +의 경우 플레이어의 기본 공격력이 14만2857이라고 가정한 딜 증가량만큼 전투력이 증가한다. 이는 앞서 엘릭서 공격력 +에서 사용된 기본 공격력과 약간 다르다.
...
반지의 경우 많은 플레이어에게 치명타 피해량이 확실하게 더 높은 딜 증가량을 보여주지만, 치명타 피해량과 치명타 적중률 모두 상옵 기준 동일하게 1.20%의 전투력을 증가시키도록 전투력 계수가 조절된 것으로 보인다.
...
|-|-|
|적중시 무공 1480, 공이속1%, 6중첩|0.240%|
|무공 9000, 체력50%이상일때 2400|0.156%|
|무공 8700, 30초마다 무공 150, 30중첩|0.121%|
